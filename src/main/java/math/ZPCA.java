...
public class ZPCA {
     ...
     * 寻找主成分
     *
     * @param primaryArray 原始二维数组数组
     * @param eigenvalue   特征值二维数组
     * @param eigenVectors 特征向量二维数组
     * @return principalMatrix 主成分矩阵
     */
    public Matrix getPrincipalComponent(double[][] primaryArray, double[][] eigenvalue, double[][] eigenVectors) {
        Matrix A = new Matrix(eigenVectors);// 定义一个特征向量矩阵
        double[][] tEigenVectors = A.transpose().getArray();// 特征向量转置
        Map<Double, double[]> principalMap = new HashMap<>();// key=主成分特征值，value=该特征值对应的特征向量
        ...
        for (int i = 0; i < plist.size(); i++) {
            if (eigenMap.containsKey(plist.get(i))) {
                principalMap.put((double) i, eigenMap.get(plist.get(i)));
            }
        }

        // 把map里的值存到二维数组里
        double[][] principalArray = new double[principalMap.size()][];
        Iterator<Map.Entry<Double, double[]>> it = principalMap.entrySet().iterator();
