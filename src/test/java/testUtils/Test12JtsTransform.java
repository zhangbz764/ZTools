...
public class Test12JtsTransform extends PApplet {
    ...
    public void setup() {
        ...
        transReflect.addReflect2D(new Vector2D(0, 0), new Vector2D(-100, 100));
        this.reflect = (LineString) transReflect.applyToGeometry2D(original);

        // multiple transform
        ZJtsTransform trans = new ZJtsTransform();
        trans.addTranslate2D(new Vector2D(200, 0)).addScale2D(2)
                .addRotateAboutPoint2D(Math.PI * 0.5, new Vector2D(200, 200))
                .addReflect2D(new Vector2D(0, 0), new Vector2D(0, 100))
        ;
