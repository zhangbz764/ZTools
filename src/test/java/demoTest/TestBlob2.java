...
public class TestBlob2 extends PApplet {
    ...
    class blob {
        ...
        void getCentroid() {
            ...
            centroid.mult((float) (1.0 / (6.0 * area)));
        }
        ...
        PVector forceFeild(PVector testPoint, boolean Newtons3rd) {
            ...
            if (equalOpposite >= 0 && Newtons3rd) {
                ...
                nodes[i].f.sub(PVector.mult(feildForce, 0.5f));
                nodes[j].f.sub(PVector.mult(feildForce, 0.5f));
            }
            return (feildForce);
        }
    ...
    public void setup() {
        ...
        for (int i = 0; i < blobs.length; i++) {
            PVector xo = new PVector(
                    ...
                    height - (radius + floor(i / blobRows) * 2 * radius * pow(3, 0.5f) / 2));
            if (floor(i / blobRows) % 2 == 0) {
                xo.x += radius;
            }
            blobs[i] = new blob(xo);
        }
